...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ★★ AB8 (2026-09-08) — LES CAPTURES DE CETTE PASSE, ET LA SEULE VÉRIFICATION
 *    QU'UNE SONDE NE PEUT PAS FAIRE.
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *   bun run abcaptures
 *   BASE_URL=http://localhost:5210 bun run abcaptures   (dry run, local build)
 *
...
const OUT = 'docs/screenshots/abpass'
...
const SHOTS: Shot[] = [
  ...
  /* ★★ AB3 · AB4 — the agenda, in its two readings: beside its map, and with
     the map hidden so the hour ladder has the whole height. */
  {
    name: 'agenda',
    hash: '#/coordinator/agenda',
    mode: { key: 'agenda', value: 'split' },
    wait: 6500,
    noRest: true,
  },
  {
    name: 'agenda-plein',
    hash: '#/coordinator/agenda',
    mode: { key: 'agenda', value: 'hidden' },
    wait: 5500,
    noRest: true,
  },
  /* ★★ AB6.6 — the export screen, its preview and its report. */
  { name: 'export', hash: '#/coordinator/export', wait: 4500, noRest: true },
  /* ★★ AB6.7 — and the association's own file, coming back. */
  { name: 'import-association', hash: '#/coordinator/import/association', wait: 4500, noRest: true },
  /* ★★ AB5b — the role switch, first block on the settings screen; and AB5a,
     the target, three blocks under it. */
...
console.log('')
console.log(`  AB8 — CAPTURES CLAIR + SOMBRE, ET LA LECTURE AU REPOS : ${BASE}`)
console.log('  ================================================================')
